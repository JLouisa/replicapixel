use crate::{
    controllers::images::ImageGenerationTrait,
...
impl ImageGenerationService {
    pub async fn generate(
        ...
        let mut credits_to_deduct = request.cost();
        let image_amount = request.num_images();

        if user_credits.credit_amount < credits_to_deduct {
            txn.rollback().await?;
        ...
        if image_amount != fal_response.amount() {
            let refund_amount = image_amount - fal_response.amount();
            credits_to_deduct -= refund_amount;
        }
